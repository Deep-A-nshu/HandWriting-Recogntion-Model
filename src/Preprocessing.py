...
def preprocessor(img, imgSize, enhance=False, dataAugmentation=False):
    ...
    fy = h / ht
    f = max(fx, fy)
    newSize = (int(max(min(wt, int(w / f)), 1)),
               int(max(min(ht, int(h / f)), 1)))  # scale according to f (result at least 1 and at most wt or ht)
...
def wer(r, h):
    # This is the Dynamic Programming problem of Edit distance.
    # initialisation
