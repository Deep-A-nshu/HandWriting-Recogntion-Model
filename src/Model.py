...
class Model:
    ...
    def __init__(self, charList, wordCharList , corpus , decoderType=DecoderType.BestPath, mustRestore=False):
        ...
        with tf.name_scope('CTC_Decoder'):
            ...
            if self.decoderType == DecoderType.BestPath:
                ...
                self.decoder = word_beam_search_module.word_beam_search(tf.nn.softmax(
                    self.ctcIn3dTBC, dim=2), 25, 'Words', 0.0, self.corpus.encode('utf8'), self.chars.encode('utf8'), self.wordChars.encode('utf8'))
    ...
    def setupTF(self):
        """ Initialize TensorFlow """
        print("\n\n\n<--------------------------- SYSTEM INFORMATION------------------------------>\n\n\n")
        print("\n<--------------------------------- WELCOME TO HANDWRITING DETECTION MODEL----------------------->\n")
